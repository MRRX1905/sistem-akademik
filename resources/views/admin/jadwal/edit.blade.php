...
<div class="container">
    ...
    <form action="{{ route('jadwal.update', $jadwal) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="mata_kuliah_id" id="mata_kuliah_id" class="form-control" required>
                ...
                @foreach($mataKuliahs as $mk)
                    <option value="{{ $mk->id }}" {{ old('mata_kuliah_id', $jadwal->mata_kuliah_id) == $mk->id ? 'selected' : '' }}>{{ $mk->nama_mk }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="ruang" id="ruang" class="form-control" value="{{ old('ruang', $jadwal->ruang) }}" required>
            @error('ruang')<div class="text-danger">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="semester" class="form-label">Semester</label>
            <input type="text" name="semester" id="semester" class="form-control" value="{{ old('semester', $jadwal->semester) }}" required>
